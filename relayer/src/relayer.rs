...
impl RelayerImpl {
    ...
    fn run_event_loop(
        ...
        let unique = Arc::new(DashSet::new());
        ...
        while !exit.load(Ordering::Relaxed) {
            crossbeam_channel::select! {
                ...
                recv(delay_packet_receiver) -> maybe_packet_batches => {
                    let start = Instant::now();
                    let failed_forwards = Self::forward_packets(maybe_packet_batches, packet_subscriptions, &slot_leaders, &mut relayer_metrics, &ofac_addresses, &unique, &address_lookup_table_cache, validator_packet_batch_size)?;
